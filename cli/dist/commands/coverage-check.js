...
export function parseCoverageSummary(json, threshold) {
    let parsed;
    try {
...
export function registerCoverageCheckCommands(program) {
    program
        .command('coverage-check')
        .description('Check test coverage against a minimum threshold (per ENGINEERING.md)')
        .option('-i, --input <path>', 'Path to Vitest coverage-summary.json', 'coverage/coverage-summary.json')
