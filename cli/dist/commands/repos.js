...
export function registerRepoCommands(program) {
    ...
    repo
        ...
        try {
            ...
            outputError('Failed to list commits', err);
            process.exit(1);
        }
    });
    // List contributors
    repo
        ...
        try {
            ...
            outputError('Failed to list contributors', err);
            process.exit(1);
        }
    });
    // Repository info
    repo
        ...
        try {
            const { owner, repo } = parseRepo(options.repo);
            const octokit = await getOctokit();
            const result = await withRetry(async () => octokit.rest.repos.get({
                owner,
                repo,
            }));
            const info = {
            ...
            outputError('Failed to get repository info', err);
            process.exit(1);
        }
    });
}
