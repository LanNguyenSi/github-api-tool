...
export function registerIssueCommands(program) {
    ...
    issue
        ...
        try {
            ...
            const result = await withRetry(async () => octokit.rest.issues.create({
                owner,
                repo,
                title: options.title,
                body: options.body,
            ...
            outputError('Failed to create issue', err);
            process.exit(1);
        }
    });
    // List issues
    issue
        ...
        try {
            ...
            const result = await withRetry(async () => octokit.rest.issues.listForRepo({
                owner,
                repo,
                state: options.state,
                labels,
            ...
            outputError('Failed to list issues', err);
            process.exit(1);
        }
    });
    // Assign issue
    issue
        ...
        try {
            ...
            const octokit = await getOctokit();
            const result = await withRetry(async () => octokit.rest.issues.addAssignees({
                owner,
                repo,
            ...
            outputError('Failed to assign issue', err);
            process.exit(1);
        }
    });
    // Comment on issue
    ...
    issue
        ...
        try {
            ...
            const issueNumber = parsePositiveInteger(options.issue, '--issue');
            const octokit = await getOctokit();
            const result = await withRetry(async () => octokit.rest.issues.update({
                owner,
                repo,
                issue_number: issueNumber,
                state: 'closed',
            ...
            outputError('Failed to close issue', err);
            process.exit(1);
        }
    });
}
