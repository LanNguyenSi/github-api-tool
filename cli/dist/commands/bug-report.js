...
const BUG_TEMPLATE = (observed, expected, reproduce, hypothesis) => `## Observed Behavior
${observed}
...
export function registerBugReportCommands(program) {
    program
        .command('bug-report')
        ...
        try {
            ...
            if (!title || !observed || !expected || !reproduce) {
                ...
                if (!observed)
                    observed = await prompt(rl, 'Observed behavior: ');
                if (!expected)
                    expected = await prompt(rl, 'Expected behavior: ');
                if (!reproduce)
                    reproduce = await prompt(rl, 'Steps to reproduce (or curl): ');
                if (!hypothesis)
                    hypothesis = await prompt(rl, 'Hypothesis (optional, Enter to skip): ');
                rl.close();
            }
